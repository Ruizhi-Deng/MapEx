...
def reselect_frontier_from_frontier_region_centers(
    frontier_region_centers, total_cost, t, start_exp_time, mission_status_save_path
):
...
def get_hector_exploration_transform_map(
    occgrid, frontiers, init_cost, mode, infogain_val_list, info_gain_weight
):
    # Check that correct mode is used
    if mode == "hector":
        assert infogain_val_list is None, "init_frontier_value must be None for hector mode"
    elif mode == "hectoraug":
        assert len(infogain_val_list) == len(frontiers), (
            "init_frontier_value must be the same length as frontiers for hector aug"
        )
    ...
    if mode == "hectoraug":
        ...
        for infogain_val in infogain_val_list:
            ...
            infogain_cost_list.append(
                int(info_gain_weight * (np.sqrt(max_infogain) - np.sqrt(infogain_val)))
            )
    ...
    for frontier_i, frontier in enumerate(frontiers):
        ...
        if mode == "hector":
            cost_map[frontier[0], frontier[1]] = (
                0  # Initialize frontier cost to 0, as it is a distance transform where frontiers are the goals
            )
        elif mode == "hectoraug":
            cost_map[frontier[0], frontier[1]] = infogain_cost_list[
                frontier_i
            ]  # Initialize frontier cost to infogain value
        else:
            raise ValueError("Invalid mode: {}".format(mode))
    ...
    while not queue.empty():
        ...
        for neighbor in neighbors:
            ...
            if (
                ...
                and (occgrid[new_x, new_y] == 0)
            ):
                new_cost = (cur_cost + 1) + init_cost[
                    new_x, new_y
                ]  # Distance to neighbor is 1 more than current
...
def gradient_planner(cur_pose, cost_transform_map):
    ...
    for neighbor_ind in neighbor_inds:
        ...
        grad_to_neighbor = (
            cost_transform_map[neighbor_pose[0], neighbor_pose[1]]
            - cost_transform_map[cur_query_pose[0], cur_query_pose[1]]
        )
...
def run_exploration_comparison_for_map(args):
    ...
    assert os.path.exists(map_occ_npy_path), "Occupancy map path does not exist: {}".format(
        map_occ_npy_path
    )
    map_valid_space_npy_path = os.path.join(map_folder_path, "valid_space.npy")
    assert os.path.exists(map_valid_space_npy_path), (
        "Valid space map path does not exist: {}".format(map_valid_space_npy_path)
    )
    ...
    if start_pose is None:
        buffer_start_pose = 2
        start_pose = smu.sample_free_position_given_buffer(
            occ_map, validspace_map, buffer_start_pose
        )
...
def run_exploration_for_map(
    ...
    try:
        ...
        cv2.imwrite(
            gt_map_save_path, smu.convert_01_single_channel_to_0_255_3_channel(mapper.gt_map)
        )
        ...
        for t in range(collect_opts.mission_time):
            ...
            if mode == "hector" or mode == "hectoraug":
                do_hector_plan = (
                    t % 1 == 0
                )  # Hector Explorer (baseline), repeat exploration every 1s
                if do_hector_plan:
                    frontier_region_centers_unscored, filtered_map, num_large_regions = (
                        frontier_planner.get_frontier_centers_given_obs_map(mapper.obs_map)
                    )
                    ...
                    if mode == "hectoraug":
                        ...
                        (
                            ...
                            hector_cur_obs_img, lama_alltrain_model, lama_map_transform, device
                        )
                        ...
                        (
                            ...
                            hector_frontier_region_centers_unscored,
                            cur_pose,
                            pose_list,
                            pred_maputils,
                            pred_vis_configs,
            ...
            if (
                mode not in ["hector", "hectoraug"]
            ):  # Non-hector methods, uses some sort of end goal to plan towards (Frontiers or UPEN RRT goals)
                ...
                if mode == "upen":  # UPEN baseline only
                    if locked_frontier_center is None:
                        ...
                        lock_frontier_center_is_invalid = not is_locked_frontier_center_valid(
                        ...
                        if need_new_locked_frontier:
                            print(
                                "UPEN: freq: {}, invalid: {}, cannot reach: {}".format(
                                    at_goal_pose_freq,
                                    lock_frontier_center_is_invalid,
                                    cannot_reach_frontier_center,
                                )
                            )
                else:
                ...
                if need_new_locked_frontier:
                    ...
                    if use_model:
                        ...
                        (
                            ...
                            cur_obs_img, lama_alltrain_model, lama_map_transform, device
                        )
                        ...
                        for model_i, model in enumerate(model_list):
                            ...
                            print(
                                "Prediction took {} seconds".format(
                                    np.round(time.time() - pred_time_start, 2)
                                )
                            )
                    ...
                    if (
                        mode == "upen"
                    ):  # not a frontier-based approach, but we will output UPEN RRT goal as a frontier to visit to use go to frontier logic
                        upen_path = None
                        while upen_path is None:
                            ...
                            ensemble = torch.zeros(
                                (
                                    lama_pred_list.shape[0],
                                    1,
                                    2,
                                    planning_grid.shape[0],
                                    planning_grid.shape[1],
                                )
                            ).cuda()
                            ensemble[:, 0, occ_chan, :, :] = lama_pred_list
                            upen_rrt_goal, upen_rrt_best_path, upen_path_dict = (
                                upen_baseline.get_rrt_goal(
                                    pose_coords=pose_coords.clone(),
                                    goal=goal_pose_coords.clone(),
                                    grid=planning_grid,
                                    ensemble=ensemble,
                                    rrt_max_iters=upen_rrt_max_iters,
                                    expand_dis=upen_expand_dis,
                                    goal_sample_rate=upen_goal_sample_rate,
                                    connect_circle_dist=upen_connect_circle_dist,
                                    rrt_num_path=upen_rrt_num_path,
                                    rrt_straight_line=upen_rrt_straight_line,
                                    reach_horizon=upen_reach_horizon,
                                    upen_mode="exploration",
                                )
                            )
                            if upen_rrt_goal is not None:
                                ...
                                print(
                                    "No intermediate goal found, using A* to get intermediate goal"
                                )
                                astar_path_to_goal = pyastar2d.astar_path(
                                    occ_grid_pyastar, cur_pose, end_pose, allow_diagonal=False
                                )
                                intermediate_goal_pose = astar_path_to_goal[
                                    np.min([20, len(astar_path_to_goal) - 1])
                                ]
                            # Use the goal as the frontier to go to
                            locked_frontier_center = intermediate_goal_pose
                            upen_path = pyastar2d.astar_path(
                                occ_grid_pyastar,
                                cur_pose,
                                locked_frontier_center,
                                allow_diagonal=False,
                        ...
                        locked_frontier_center = frontier_region_centers[
                        ...
                        while not is_locked_frontier_center_valid(
                            ...
                            (
                                ...
                                frontier_region_centers,
                                frontier_cost_list,
                            ) = reselect_frontier_from_frontier_region_centers(
                                frontier_region_centers,
                                frontier_cost_list,
                                t,
                                start_exp_time,
                ...
                if do_hector_plan:  # Do Hector cost map calculation every 1 time steps
                    ...
                    obstacle_transform_map = cv2.distanceTransform(
                        obstacle_transform_map.astype(np.uint8), cv2.DIST_L2, cv2.DIST_MASK_PRECISE
                    )
                    # Follow Eq.5 of the Hector Explorer paper
                    obs_ramp_dist = 15
                    obs_min_dist = 2
                    danger_transform_map = np.zeros(mapper.obs_map.shape)
                    danger_transform_map[obstacle_transform_map < obs_ramp_dist] = (
                        obs_ramp_dist
                        - obstacle_transform_map[obstacle_transform_map < obs_ramp_dist]
                    ) * 5
                    ...
                    if mode == "hector":
                        ...
                        init_frontier_value = (
                            -1 * frontier_cost_list
                        )  # Initialize to the frontier value cost as in Enhanced Exploration with Predicted Map paper. Need to negate since we want to pass in as higher information gain is higher
                    else:
                        raise ValueError(
                            "Invalid mode: {}, mode not available for getting init_frontier_value".format(
                                mode
                            )
                        )
                    cost_transform_map = get_hector_exploration_transform_map(
                        mapper.obs_map,
                        frontier_region_centers,
                        init_cost=danger_transform_map,
                        mode=mode,
                        infogain_val_list=init_frontier_value,
                        info_gain_weight=200,
                    )
            ...
            if chosen_local_planner == "astar":
                ...
                for _ in range(
                    ind_to_move_per_step
                ):  # Move the same steps as psuedo_traj_controller + A*
            ...
            if show_plt:
                ...
                if pose_list is not None:
                    ax_gt.plot(
                        pose_list[:, 1] - pd_size, pose_list[:, 0] - pd_size, c="r", alpha=0.5
                    )
                    ax_gt.scatter(
                ...
                ax_obs.imshow(
                    mapper.obs_map[pd_size:-pd_size, pd_size:-pd_size], cmap=cmap
                )  # , **map_kwargs)

                if pose_list is not None:
                    ax_obs.plot(
                        pose_list[:, 1] - pd_size, pose_list[:, 0] - pd_size, c="#417CF2", alpha=1.0
                    )
                ...
                if mode not in [
                    ...
                    "hector",
                    "hectoraug",
                ]:  # UPEN and Hector do not have locked frontiers
                    ax_obs.scatter(
                ...
                if mode not in [
                    ...
                    "hector",
                    "hectoraug",
                ]:  # UPEN and Hector are not a frontier planner
                    if viz_medium_flooded_grid is not None:
                        second_most_flooded_grid = viz_medium_flooded_grid[
                            pd_size:-pd_size, pd_size:-pd_size
                        ]
                    ...
                    if viz_most_flooded_grid is not None:
                        most_flooded_grid = (
                            viz_most_flooded_grid  # [pd_size:-pd_size,pd_size:-pd_size]
                        )
                        flooded_ind = np.where(most_flooded_grid == True)
                        flooded_ind_colors_alpha = np.zeros(
                            (mapper.obs_map.shape[0], mapper.obs_map.shape[1], 4)
                        )
                        flooded_ind_colors_alpha[flooded_ind[0], flooded_ind[1], :] = (
                            255 / 255,
                            159 / 255,
                            28 / 255,
                            0.3,
                ...
                if pred_maputils is not None:
                    ...
                    occupied_indices_in_obsmap = np.where(
                        mapper.obs_map[pd_size:-(pd_size), pd_size:-(pd_size)] == 1.0
                    )  # indices where obs_map is occupied
                    obs_occ_mask[occupied_indices_in_obsmap] = 1
                    obs_occ_mask_colors = ["#000000", "#000000"]
                    obs_occ_mask_cmap = LinearSegmentedColormap.from_list(
                        "mask_black", obs_occ_mask_colors, N=2
                    )
                    ...
                    ax_pred.imshow(
                        obs_occ_mask, cmap=obs_occ_mask_cmap, alpha=obs_occ_mask_alpha
                    )  # obs_map known occ cells -> black

                    # overlay unknown(obs_map) cells as gray tint
                    obs_unk_mask = np.zeros_like(
                        pred_maputils[
                            pd_size + pad_h1 : -(pd_size + pad_h2),
                            pd_size + pad_w1 : -(pd_size + pad_w2),
                        ]
                    )
                    unknown_indices_in_obs_map = np.where(
                        mapper.obs_map[pd_size:-(pd_size), pd_size:-(pd_size)] == 0.5
                    )  # indices of unknown cells in obs_map
                    obs_unk_mask[unknown_indices_in_obs_map] = 1
                    grey = (
                        "#909090"  # tunable grey value #606060 might be better for pred_maputils?
                    )
                    obs_unk_mask_colors = [grey, grey]
                    obs_unk_mask_cmap = LinearSegmentedColormap.from_list(
                        "mask_grey", obs_unk_mask_colors, N=2
                    )
                    obs_unk_mask_alpha = np.zeros_like(obs_unk_mask, dtype=float)
                    obs_unk_mask_alpha[obs_unk_mask == 1] = (
                        0.3  # tunable opacity for grey unknown area
                    )
                    ...
                    if mode not in ["hectoraug"]:  # Hector aug does not have variance
                        ...
                        colors = [white, orange]
                        n_bins = 10
                        var_cmap = LinearSegmentedColormap.from_list(
                            "custom_green_var", colors, N=n_bins
                        )
                        ax_pred_var.imshow(
                            var_map[
                                pd_size + pad_w1 : -(pd_size + pad_w2),
                        ...
                        if mean_map is not None:
                            ...
                            cmap = LinearSegmentedColormap.from_list(
                                "customgreenred", colors, N=n_bins
                            )
                            ax_mean_map.imshow(
                                mean_map[pd_size:-(pd_size), pd_size:-(pd_size)], cmap=cmap
                            )  # predicted map

                            # overlay observed(known) occupied cells on top of the predicted map
                            obs_occ_mask = np.zeros_like(
                                mean_map[pd_size:-(pd_size), pd_size:-(pd_size)]
                            )
                            occupied_indices_in_obsmap = np.where(
                                mapper.obs_map[
                                    pd_size - pad_h1 : -(pd_size - pad_h2),
                            ...
                            obs_unk_mask_alpha[obs_unk_mask == 0] = 0.0
                            ax_mean_map.imshow(
                                obs_unk_mask, cmap=obs_unk_mask_cmap, alpha=obs_unk_mask_alpha
                            )
                            ...
                            if mode not in [
                                "hector",
                                "hectoraug",
                            ]:  # Hector does not have path planning
                                ax_mean_map.plot(
                            ...
                            if mode != "upen":
                                if viz_most_flooded_grid is not None:
                                    ...
                                    flooded_ind = np.where(most_flooded_grid == True)
                                    flooded_ind_colors_alpha = np.zeros(
                                        (mean_map.shape[0], mean_map.shape[1], 4)
                                    )
                                    flooded_ind_colors_alpha[flooded_ind[0], flooded_ind[1], :] = (
                                        255 / 255,
                                        159 / 255,
                                        28 / 255,
                                        0.3,
                                    )  # orange color for visibility mask
                                    ax_mean_map.imshow(
                                        flooded_ind_colors_alpha[pd_size:-pd_size, pd_size:-pd_size]
                                    )
                                if viz_medium_flooded_grid is not None:
                                    medium_flooded_ind = np.where(viz_medium_flooded_grid == True)
                                    medium_flooded_ind_colors_alpha = np.zeros(
                                        (mean_map.shape[0], mean_map.shape[1], 4)
                                    )
                                    # medium_flooded_ind_colors_alpha[medium_flooded_ind[0], medium_flooded_ind[1],:] = (154/255,230/255,72/255,0.4)
                                    medium_flooded_ind_colors_alpha[
                                        medium_flooded_ind[0], medium_flooded_ind[1], :
                                    ] = (207 / 255, 3 / 255, 252 / 255, 0.25)
                ...
                if mode in ["hector", "hectoraug"]:
                    ax_mean_map.imshow(
                        cost_transform_map[pd_size:-pd_size, pd_size:-pd_size], cmap="gray"
                    )
            ...
            # Save obs map
            cv2.imwrite(
                global_obs_dir + "/{}.png".format(str(t).zfill(8)),
            ...
            # if (t % 99) == 0:
            print(
                "Total time for step {} is {} seconds".format(
                    t, np.round(time.time() - start_mission_i_time, 2)
                )
            )
...
if __name__ == "__main__":
    ...
    assert not (collect_opts.test_world_only and collect_opts.collect_world_list is not None), (
        "Only one of test_world_only and collect_world_list can be true"
    )
    ...
    output_root_dir = os.path.join(
        collect_opts.root_path, collect_opts.output_folder_name, output_subdirectory_name
    )
    ...
    if collect_opts.ensemble_folder_name is not None:
        ...
        ensemble_model_dirs = sorted(
            os.listdir(
                os.path.join(collect_opts.root_path, "pretrained_models", ensemble_folder_name)
            )
        )
    ...
    lama_model = load_lama_model(
        os.path.join(
            collect_opts.root_path, "pretrained_models", collect_opts.big_lama_model_folder_name
        ),
        device=collect_opts.lama_device,
    )
